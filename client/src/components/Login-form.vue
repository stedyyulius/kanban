    ...
    <section id="login">
      <div class="container">
      	<div class="row">
      	    <div class="col-xs-12">
          	    <div class="form-wrap">
                  <h1>Welcome to Hacktiv Kanban</h1>
...
export default {
  ...
  methods:{
    ...
    login(){
      let self = this;
      // self.$db.ref('user').child(`${self.username}`).once("value")
      // .then(function(data) {
      //   console.log(data.val());
      //   });
      // 
        self.$db.ref('user').orderByChild('username').equalTo(`${self.username}`).on("child_added", function(username) {
            if(username){
              self.$db.ref('user').orderByChild('password').equalTo(`${self.password}`).on("child_added",function(token){
                if(token){
                  alert(`Login Success!`)
